/*-------------------------------------------------------------------------------

	BARONY
	File: monster_flyingrat.cpp
	Desc: implements all of the Flying Rat monster's code

	Copyright 2013-2016 (c) Turning Wheel LLC, all rights reserved.
	See LICENSE for details.

-------------------------------------------------------------------------------*/

#include "main.hpp"
#include "game.hpp"
#include "stat.hpp"
#include "entity.hpp"
#include "monster.hpp"
#include "engine/audio/sound.hpp"
#include "items.hpp"
#include "net.hpp"
#include "collision.hpp"
#include "prng.hpp"

void initFlyingRat(Entity* my, Stat* myStats)
{
	my->flags[BURNABLE] = true;
	my->flags[UPDATENEEDED] = true;
	my->flags[INVISIBLE] = false;

	my->initMonster(1232);

	if ( multiplayer != CLIENT )
	{
		MONSTER_SPOTSND = 29;
		MONSTER_SPOTVAR = 1;
		MONSTER_IDLESND = 29;
		MONSTER_IDLEVAR = 1;
	}

			// generate 6 items max, less if there are any forced items from boss variants
			int customItemsToGenerate = ITEM_CUSTOM_SLOT_LIMIT;

			// random effects

			// generates equipment and weapons if available from editor
			createMonsterEquipment(myStats);

			// create any custom inventory items from editor if available
			createCustomInventory(myStats, customItemsToGenerate);

			// count if any custom inventory items from editor
			int customItems = countCustomItems(myStats); //max limit of 6 custom items per entity.

			// count any inventory items set to default in edtior
			int defaultItems = countDefaultItems(myStats);

			my->setHardcoreStats(*myStats);

			// generate the default inventory items for the monster, provided the editor sprite allowed enough default slots
			switch (defaultItems)
			{
			case 6:
			case 5:
			case 4:
			case 3:
			case 2:
			case 1:
				if (local_rng.rand() % 4)
				{
					if (local_rng.rand() % 2)
					{
						newItem(FOOD_MEAT, EXCELLENT, 0, 1, local_rng.rand(), false, &myStats->inventory);
					}
					else
					{
						newItem(FOOD_CHEESE, DECREPIT, 0, 1, local_rng.rand(), false, &myStats->inventory);
					}
				}
				break;
			default:
				break;
			}
	{
	myStats->EFFECTS[EFF_LEVITATING] = true;
	myStats->EFFECTS_TIMERS[EFF_LEVITATING] = 0;
	}
}

float myZ = 0;
int bobCount = 0;

void flyingRatAnimate(Entity* my, double dist)
{
    int spriteStart = 1232;
	if (my->ticks == my->getUID() % TICKS_PER_SECOND) {
		if (multiplayer == SERVER) {
			// in case we are algernon, update the sprite for clients
			serverUpdateEntitySprite(my);
		}
	}

	float myZOffset = -3.0 + sin(my->ticks * 0.125) * 2;
	bobCount = (bobCount + 1) % 6000;

	// walk cycle
	if (dist >= 0.1 && !MONSTER_ATTACK) {
		if (my->ticks % 10 == 0)
		{
			// normal rat walk cycle
			if (my->sprite == spriteStart + 0) {
				my->sprite = spriteStart + 1;
			}
			else if (my->sprite == spriteStart + 1) {
				my->sprite = spriteStart + 0;
			}
		}
	}

	static ConsoleVariable<bool> cvar_useFocalZ("/flyingrat_anim_use_focal_z", false);

	// attack cycle
	if (MONSTER_ATTACK) {
		const int frame = TICKS_PER_SECOND / 10;
		if (MONSTER_ATTACKTIME == frame * 0) { // frame 1
			my->sprite = spriteStart + 2;
		}
		if (MONSTER_ATTACKTIME == frame * 1) { // frame 2
			my->sprite =  spriteStart + 3;
		}
		if (MONSTER_ATTACKTIME == frame * 2) { // frame 3
			my->sprite = spriteStart + 4;
		}
		if (MONSTER_ATTACKTIME == frame * 4) { // frame 4
			my->sprite = spriteStart + 5;
			const Sint32 temp = MONSTER_ATTACKTIME;
			my->attack(1, 0, nullptr); // munch
			MONSTER_ATTACKTIME = temp;
		}
		if (MONSTER_ATTACKTIME == frame * 6) { // frame 5
			my->sprite = spriteStart + 6;
		}
		if (MONSTER_ATTACKTIME == frame * 7) { // end
            my->sprite = spriteStart + 0;
			MONSTER_ATTACK = 0;
			MONSTER_ATTACKTIME = 0;
		}
		else {
			++MONSTER_ATTACKTIME;
			my->new_z = myZ;
		}
	}
	my->z = myZ + myZOffset;
}

	void flyingRatDie(Entity * my)
{
	Entity* gib = spawnGib(my);
	gib->skill[5] = 1; // poof
	gib->sprite = my->sprite;
	gib->pitch = 0.0;
	serverSpawnGibForClient(gib);
	for ( int c = 0; c < 5; c++ )
	{
		Entity* gib = spawnGib(my);
		serverSpawnGibForClient(gib);
	}

	my->spawnBlood();

	playSoundEntity(my, 30, 64);

	my->removeMonsterDeathNodes();

	list_RemoveNode(my->mynode);
	return;
}
